...
namespace Utilities
{
	static constexpr int MaxNumThreads = 8;
									...
									for (int endIndex = (beginIndex + 1) % ProfilerMarkerBufferSize; endIndex != profilerNextWriteIndex[l]; endIndex = (endIndex + 1) % ProfilerMarkerBufferSize)
									{
										...
										if (endMarker.type == MarkerType::END_FUNCTION && beginMarker.identifier == endMarker.identifier)
										{
											...
											else
											{
												...
												exclusiveTime += std::chrono::duration_cast<std::chrono::nanoseconds>(endMarker.timePoint - lastInterruptionEnd->timePoint).count();
										...
										else if (endMarker.IsEndMark() && (lastInterruptionBegin == nullptr || endMarker.identifier == lastInterruptionBegin->identifier))
										{
											...
											else
											{
												...
												exclusiveTime = std::chrono::duration_cast<std::chrono::nanoseconds>(endMarker.timePoint - beginMarker.timePoint).count();
	...
	private:
		...
		float millisecondLength = 140.0f;
	};
}
#endif